...
async def _(event: GroupMessageEvent):
    ...
    await TZtreasury.set(group, (golds - gold))
...
@gold.handle()
async def _( event: GroupMessageEvent):
    my = await TZtreasury.get(group_id=event.group_id)
    await gold.finish(f"{NICKNAME}的金库中存有{my}枚金币")
...
async def _( event: GroupMessageEvent,arg: Message = CommandArg()):
    ...
    await TZtreasury.add(group_id=event.group_id,num = num)
